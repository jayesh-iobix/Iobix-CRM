...
const EditGtmServiceClient = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.companyGstNumber)
      newErrors.companyGSTNumber = "Company GSt Number is required";
    if (!formData.fullName)
      newErrors.fullName = "Contact Person Name is required";
    if (!formData.companyLinkedin)
      newErrors.companyLinkedin = "Company Linkedin is required";
    if (!formData.pointOfContact)
      newErrors.pointOfContact = "Point Of Contact Person Name is required";
    ...
    if (!formData.personalEmail)
      newErrors.personalEmail = "Person Email is required";
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        const response = await GtmClientService.updateGtmClient(
          id,
          companyData
        ); // Call the service
        ...
        if (response.status === 2) {
          toast.error(
            "This Email Is Already Registered, Please Enter Another Valid Email"
          ); // Toast on error
        }
    ...
    if (validate()) {
      ...
      if (secondFormData.gtmClientServiceId) {
        ...
        formDataToSend.append(
          "GtmClientServiceId",
          secondFormData.gtmClientServiceId
        );
      } else {
      ...
      if (secondFormData.agreementDocument instanceof File) {
        formDataToSend.append(
          "AgreementDocument",
          secondFormData.agreementDocument?.name || ""
        );
      } else {
        formDataToSend.append(
          "AgreementDocument",
          secondFormData.agreementDocument
        );
      }
  ...
  const validateInvoiceForm = () => {
    ...
    if (!thirdFormData.invoiceDetailId)
      newErrors.invoiceDetailId = "Issue By Company is required";
    ...
    if (validateInvoiceForm()) {
      ...
      try {
        ...
        if (thirdFormData.gtmInvoiceConnectionId) {
          const response = await GtmConnectionService.updateGtmConnection(
            thirdFormData.gtmInvoiceConnectionId,
            thirdFormData
          ); // Call the service
          if (response.status === 1) {
          ...
          const response = await GtmConnectionService.addGtmConnection(
            gtmConnectionData
          ); // Call the service
          if (response.status === 1) {
            toast.success("Invoice Updated Successfully");
          ...
          <form onSubmit={handleInvoiceSubmit} className="container">
            <div className="-mx-4 px-10 mt- flex flex-wrap">
                  ...
                  {invoiceDetailList.length > 0 ? (
                    invoiceDetailList.map((invoiceDetail) => (
                      <option
                        ...
                        value={invoiceDetail.invoiceDetailId}
                      >
              ...
              <div className="w-full mb-2 px-3">
                ...
                {(thirdFormData?.allProducts?.length
                  ? thirdFormData.allProducts
                  : [{ description: "", quantity: "", unit: "", unitPrice: "" }]
                ).map((item, index) => (
