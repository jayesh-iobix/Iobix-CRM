...
//#region Component: AddTaxDetails
const AddTaxDetails = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        const response = await TaxDetailService.addTaxDetail(taxDetailData);
        if (response.status === 1) {
          navigate(-1);
          toast.success("Tax Detail Added Successfully"); // Toast on success
        }
        // Reset the form
        setTaxName('');
        setPercentage('');
    ...
    return (
      <>
        ...
        <div className="flex justify-between items-center my-3">
          <h1 className="font-semibold text-2xl">Add Tax Detail</h1>
