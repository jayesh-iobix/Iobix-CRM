...
const EditInquiryOrigin = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // console.log(inquiryOrigin.data);
      setInquiryOriginName(inquiryOrigin.data.inquiryOriginName);
      setIsActive(inquiryOrigin.data.isActive); // Assuming the department object contains isActive
    };
    fetchData();
  }, [id]);
  //#endregion
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        const response = await InquiryOriginService.updateInquiryOrigin(id, inquiryOriginData);
        ...
        console.error('Error Editing inquiry origin:', error);
        alert('Failed to edit inquiry origin.');
      }
          ...
          <div className="-mx-4 px-10 mt- flex flex-wrap">
            <div className="w-full mb-2 px-3 md:w-1/2 lg:w-1/2">
              ...
              {errors.inquiryOriginName && (
                <p className="text-red-500 text-xs">{errors.inquiryOriginName}</p>
              )}
            </div>
