...
const AddDesignation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await DesignationService.addDesignation(designationData);
  ...
  return (
    <>
      <div className="flex justify-between items-center my-3">
        ...
        <Link to='/master/designation-list' className="bg-gray-500 hover:bg-gray-400 text-white font-bold py-2 px-4 rounded flex items-center gap-2 hover:no-underline">
          <FaArrowLeft size={16} />
          Back
        </Link>
      </div>
          ...
          <div className='-mx-4 px-10 mt- flex flex-wrap'>
            <div className='w-full mb-2 px-3 md:w-1/2 lg:w-1/2'>
              ...
              <div className='relative z-20'>
                <select value={departmentId} onChange={(e) => setDepartmentId(e.target.value)} className='relative z-20 w-full mb-2 appearance-none rounded-lg border border-stroke bg-transparent py-[10px] px-4 text-dark-6 outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-2'>
                  <option value='' className='text-gray-400'>
                    --Select Department--
                  </option>
                  {departmentList.length > 0 ? (
                    departmentList.map((departmentItem) => (
                      <option key={departmentItem.departmentId} value={departmentItem.departmentId}>
            ...
            <div className='w-full mb-2 px-3 md:w-1/2 lg:w-1/2'>
              ...
              />
              {errors.designationName && <p className="text-red-500 text-xs">{errors.designationName}</p>}
            </div>
