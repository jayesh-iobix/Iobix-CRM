...
const CompanyRegistration = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await ClientCompanyService.addClientCompany(companyData);
      if (response.status === 1) {
        toast.success("Registration successfull! Please check the register email for next steps."); // Toast on success
